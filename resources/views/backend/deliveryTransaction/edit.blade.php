...
@section('content')
	<form role="form" method="post" action="{{ route('delivery-transaction.update', $deliveryTransaction->id) }}">
		<div class="row">
      @csrf
			@method('patch')
	    <!-- left column -->
	      ...
	      <div class="card card-primary">
            ...
            <div class="card-tools" style="display: flex;">
              <div style="margin-right: 30px;">
                ...
                <a href="{{ route('delivery-transaction.create') }}" class="btn btn-success btn-xs" title="Add Delivery Transaction">Add Delivery Transaction</a>
              </div>
            </div>
                ...
                <div class="form-group">
                  ...
                  <select class="form-control" name="customer_id">
                    ...
                    @foreach ($customers as $customer)
                      <option value="{{ $customer->id }}" {{ ($deliveryTransaction->customer_id==$customer->id)? 'selected' : '' }}>{{ $customer->name }}</option>
                    @endforeach
                ...
                <div class="form-group">
                  ...
                  <select class="form-control" name="zone_id" id="zone_id">
                    ...
                    @foreach ($zones as $zone)
                      <option value="{{ $zone->id }}" {{ ($zone->id==$deliveryTransaction->zone_id)? 'selected' : '' }}>{{ $zone->name }}</option>
                    @endforeach
              ...
              <div class="col-md-12">
                <div class="form-group">
                  ...
                  <select class="form-control" name="car_id" id="car_id">
                    ...
                    @foreach ($cars as $car)
                      <option value="{{ $car->id }}" {{ ($deliveryTransaction->car_id==$car->id)? 'selected' : '' }}>{{ $car->display_name }}</option>
                    @endforeach
  ...
  <script type="text/javascript">
    ...
    @if (!empty($deliveryTransaction->location_id))
      var zoneId = '{{ $deliveryTransaction->zone_id }}';
      var url = '{{ url('/') }}';
      var locationId = '{{ $deliveryTransaction->location_id }}';
      // console.log(url);
    ...
    @if (!empty($deliveryTransaction->car_id))
      var carId = '{{ $deliveryTransaction->car_id }}';
      var carModelId = '{{ $deliveryTransaction->car_model_id }}';
      var url = '{{ url('/') }}';
      ...
      $.ajax({
        ...
        .done(function( msg ) {
          ...
          for(var i = 0; i < msg.length; i++){
            data = data + '<option value="'+ msg[i]['id'] +'"'+ ((carModelId==msg[i]['id'])? 'selected' : '') +'>'+ msg[i]['display_model_name']+ '</option>';
